...
def index(request):
    ...
    num_bulgakov_books = Book.objects.filter(author__last_name__icontains='bulgakov').count
    ...
    return render(
        ...
        'num_genres':num_genres, 'num_bulgakov_books':num_bulgakov_books, 'num_visits':num_visits},
    )
...
class LoanedBooksByAllListView(PermissionRequiredMixin,generic.ListView):
    ...
    permission_required = ('catalog.can_see_borrowers')
    model = BookInstance
    template_name ='catalog/bookinstance_list_borrowed_library.html'
    paginate_by = 10
